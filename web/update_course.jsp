...
<html lang="vi">
        ...
        <div class="update-course-page">
            <section class="container py-5">
                ...
                <form id="wizardForm" method="post" action="EditCourseServlet" enctype="multipart/form-data" class="bg-white p-4 rounded-4 shadow-sm">
                    ...
                    <input type="hidden" id="currentLessonQuizIndex" name="currentLessonQuizIndex" value="" />
                    <!-- THÔNG TIN CHUNG -->
        ...
        <div class="modal fade" id="quizModal" tabindex="-1" aria-labelledby="quizModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form id="quizForm">
                        <div class="modal-header">
                            <h5 class="modal-title" id="quizModalLabel">Sửa Quiz</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
    ...
    <script>
        ...
        var allCourseData;
        try {
            allCourseData = JSON.parse('${quizDataJson}'.replace(/\\u0027/g, "'").replace(/\\u0022/g, '"'));
            console.log('Successfully parsed allCourseData:', allCourseData);

            // Validate structure
            if (allCourseData && allCourseData.lessons) {
                console.log('Found', allCourseData.lessons.length, 'lessons');
                allCourseData.lessons.forEach((lesson, index) => {
                    if (lesson.quizzes) {
                        console.log(`Lesson ${index} has ${lesson.quizzes.length} quizzes:`, lesson.quizzes);
                    } else {
                        console.log(`Lesson ${index} has no quizzes`);
                    }
                });
            }
        } catch (error) {
            console.error('Error parsing quizDataJson:', error);
        ...
        allCourseData.hidden = ${course.hidden};
        allCourseData.suggested = ${course.suggested};
        console.log('Final allCourseData:', allCourseData);
    </script>
