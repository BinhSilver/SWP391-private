...
-- 4. Payments (đủ trường, tích hợp PayOS)
CREATE TABLE Payments (
    ...
    OrderCode NVARCHAR(100),     -- PayOS
    CheckoutUrl NVARCHAR(255),   -- PayOS
    Status NVARCHAR(20),         -- PayOS ('PAID', 'PENDING', ...)
    CreatedAt DATETIME DEFAULT GETDATE(),
...
-- 9. Lesson Materials (giữ luôn FileType vì 1 bản có, 1 bản không)
...
-- 12. Kanji (có LessonID luôn, khỏi ALTER sau)
CREATE TABLE Kanji (
...
-- 16. Questions & Answers (thêm TimeLimit luôn, khỏi ALTER)
...
-- 21. Rooms & RoomParticipants (chuẩn SQL Server)
CREATE TABLE Rooms (
    ...
    MaxAge INT,
    AllowApproval BIT DEFAULT 0,     -- Giống BOOLEAN của MySQL
    IsActive BIT DEFAULT 1,
    CreatedAt DATETIME DEFAULT GETDATE(),
...
BEGIN
    UPDATE Users
    SET Avatar = CASE
                    WHEN i.Gender = 'Nữ' THEN 'img/nu.jpg'
                    ELSE 'img/nam.jpg'
                END
    FROM Users u
